...
export async function handleProjectRequest(req: IncomingMessage, res: ServerResponse, id?: string): Promise<void> {
  const db = getDb();
  if (id) {
    if (req.method === "GET") {
    ...
    if (req.method === "PATCH") {
      ...
      const map: Record<string, string> = { title: "title", notes: "notes", type: "type", archived: "archived", sort_order: "sortOrder", repo_path: "repoPath", github_repo: "githubRepo" };
    ...
    if (req.method === "DELETE") { db.delete(projects).where(eq(projects.id, id)).run(); return json(res, { deleted: true }); }
  ...
  if (req.method === "POST") {
    ...
    db.insert(projects).values({ id: pid, title: body.title as string, type: body.type as string, notes: (body.notes as string) ?? null, createdAt: now, updatedAt: now }).run();
