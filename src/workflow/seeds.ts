    ...
    nodes: [
      {
        ...
        config: {
          ...
          goto_if: [
            { match: "true", goto: "route_by_agent" },
          ],
          default: "route_by_agent",
        },
      },
      {
      ...
      },
      {
        ...
        type: "tool_call",
        config: {
          command: 'if [ -z "{project.repo_path}" ] || [ ! -d "{project.repo_path}/.git" ]; then echo "no git repo — skipping commit"; else cd {project.repo_path} && git add -A && (git diff --cached --quiet && echo "nothing to commit" || (git commit -m "agent(programmer): {task.title}" --author "lobs <$(git config user.email || echo lobs@localhost)>" && git push)); fi',
          timeout_seconds: 60,
        },
      ...
      },
      {
        ...
        type: "tool_call",
        config: {
          command: 'if [ -z "{project.repo_path}" ] || [ ! -d "{project.repo_path}/.git" ]; then echo "no git repo — skipping commit"; else cd {project.repo_path} && git add -A && (git diff --cached --quiet && echo "nothing to commit" || (git commit -m "agent(programmer): fix - {task.title}" --author "lobs <$(git config user.email || echo lobs@localhost)>" && git push)); fi',
          timeout_seconds: 60,
        },
      ...
      },
      {
        ...
        type: "tool_call",
        config: {
          command: 'if [ -z "{project.repo_path}" ] || [ ! -d "{project.repo_path}/.git" ]; then echo "no git repo — skipping commit"; else cd {project.repo_path} && git add -A && (git diff --cached --quiet && echo "nothing to commit" || (git commit -m "agent(programmer): fix2 - {task.title}" --author "lobs <$(git config user.email || echo lobs@localhost)>" && git push)); fi',
          timeout_seconds: 60,
        },
      ...
      },
      {
        ...
        config: {
          agent_type: "reviewer",
          model_tier: "small",
          prompt_template: "Review the programmer output for this task.\n\nTask: {task.title}\n\nNotes:\n{task.notes}\n\nRepo: {project.repo_path}\n\nCheck for: missing tests, missing README, obvious bugs, correctness. Be direct and actionable.",
          timeout_seconds: 600,
        },
        on_success: "done",
        on_failure: { retry: 0 },
      ...
      },
      {
        ...
        type: "tool_call",
        config: {
          command: 'cd {project.repo_path} && git add -A && (git diff --cached --quiet && echo "nothing to commit" || (git commit -m "agent(architect): {task.title}" --author "lobs <$(git config user.email || echo lobs@localhost)>" && git push))',
          timeout_seconds: 60,
        },
        on_success: "done",
        on_failure: { retry: 0 },
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        type: "tool_call",
        config: {
          command: "cd {project.repo_path} && git add -A && git diff --cached --quiet || git commit -m 'agent(programmer): {task.title}' --author 'lobs <$(git config user.email || echo lobs@localhost)>'",
          timeout_seconds: 30,
        },
        on_success: "cleanup",
        on_failure: { retry: 1 },
