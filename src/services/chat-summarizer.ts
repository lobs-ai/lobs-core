...
async function callLocalModel(
  ...
  try {
    ...
    console.error(`[DEBUG-LLM] calling ${baseUrl}/chat/completions with model=${model}`);
    console.error(`[DEBUG-LLM] messages count: ${requestBody.messages.length}`);
    ...
    console.error(`[DEBUG-LLM] response status: ${response.status}`);
    if (!response.ok) {
    ...
    content = content.replace(/<think>[\s\S]*?<\/think>/g, "").trim();
    console.error(`[DEBUG-LLM] final content: "${content}"`);
...
export async function generateChatTitle(sessionKey: string): Promise<string | null> {
  const db = getDb();
  console.error(`[DEBUG-TITLE] generateChatTitle called for ${sessionKey}`);
  ...
  try {
    ...
    console.error(`[DEBUG-TITLE] session found: ${!!session}, label: ${session?.label}`);
    ...
    console.error(`[DEBUG-TITLE] isDefaultTitle: ${isDefaultTitle}, label: "${session.label}"`);
    if (!isDefaultTitle) {
...
export function onUserMessage(sessionKey: string): void {
  console.error(`[DEBUG-SUMMARIZER] onUserMessage called for ${sessionKey}`);
  (async () => {
    try {
      console.error(`[DEBUG-SUMMARIZER] calling generateChatTitle for ${sessionKey}`);
      const title = await generateChatTitle(sessionKey);
      console.error(`[DEBUG-SUMMARIZER] generateChatTitle returned: ${JSON.stringify(title)}`);
      if (title) emitTitleUpdate(sessionKey, title);
    } catch (err) {
      console.error(`[DEBUG-SUMMARIZER] onUserMessage hook FAILED: ${err}`);
