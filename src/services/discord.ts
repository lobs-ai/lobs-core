...
/** Agent names for plain-text mention detection — loaded dynamically from SOUL.md files */
const getAgentNames = () => getAgentMentionNames();
...
class DiscordService {
  ...
  private setupMessageListener(handler: (message: {
    ...
    this.client.on("messageCreate", async (msg) => {
      ...
      console.log(`[discord] messageCreate guild=${msg.guildId} channel=${msg.channelId} author=${msg.author.tag}(${msg.author.id}) bot=${msg.author.bot} content=${JSON.stringify(msg.content.slice(0,80))}`);
      // Ignore bot messages unless they're in the allow list or @mention us
      if (msg.author.bot && !this.config!.botAllowFrom.includes(msg.author.id) && !msg.mentions.has(this.client!.user!)) return;
      ...
      if (!msg.guildId) {
        ...
        if (!policy || !policy.allow) {
          console.log(`[discord] Dropping message from disallowed channel ${msg.channelId} guild=${msg.guildId}`);
          return; // Silently drop messages from disallowed channels
        }

        // Check mention requirement — accept @mention OR plain text name (case-insensitive)
        ...
        if (policy.requireMention && !isMentioned) {
          console.log(`[discord] Dropping message from ${msg.channelId} (requireMention) content=${msg.content.slice(0,60)}`);
