Add a health check endpoint to the lobs-memory server

## Problem
The lobs-memory server at ~/lobs-memory needs a simple /healthz endpoint that returns
a JSON response with uptime and basic stats.

## Acceptance Criteria
- [ ] GET /healthz returns { status: "ok", uptime: <seconds> }
- [ ] Response includes document and chunk counts
- [ ] Endpoint responds in < 100ms

## Constraints
- Don't modify the existing /status endpoint
- Keep it simple — no external dependencies
- Add it near the other route handlers in server/index.ts

## Context
- Server is a Bun TypeScript project at ~/lobs-memory
- Routes are in server/index.ts
- There's already a /status endpoint — this is a lighter alternative
